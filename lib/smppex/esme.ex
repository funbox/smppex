defmodule SMPPEX.ESME do
  ...
  @doc """
  Starts an `SMPPEX.Session` implementation as an ESME entitiy, i.e. makes a
  transport connection to `host`:`port` and starts an `SMPPEX.Session` to handle
  the connection with the passed module.
  ...
  * `:esme_opts` is a keyword list of ESME options:
  ...
  defp convert_host(host) when is_binary(host), do: SMPPEX.Compat.to_charlist(host)
