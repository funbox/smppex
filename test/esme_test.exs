defmodule SMPPEX.ESMETest do
  ...
  test "start_link" do
    ...
    assert {:ok, _} = ESME.start_link({127,0,0,1}, Server.port(server), {SupportSession, {pid, handler}})
  end
  ...
  test "start_link by hostname" do
    ...
    assert {:ok, _} = ESME.start_link('localhost', Server.port(server), {SupportSession, {pid, handler}})
  end
  ...
  test "start_link when MC is down" do
    ...
    assert {:error, :econnrefused} = ESME.start_link("localhost", port, {SupportSession, {pid, handler}})
  end
